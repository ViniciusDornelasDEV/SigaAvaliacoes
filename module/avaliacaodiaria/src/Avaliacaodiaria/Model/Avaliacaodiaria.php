...
class Avaliacaodiaria Extends BaseTable {

    //RECUPERAR AVALIAÇÔES COM PERÍODO EM ABERTO PARA OPERADOR DIÁRIA
	public function getAvaliacoesRespondidas($params){
        return $this->getTableGateway()->select(function($select) use ($params) {
            $select->join(
                array('e' => 'tb_empresa'), 
                'e.id = tb_avaliacaodiaria.empresa', 
            ...
            $select->join(
                array('o' => 'tb_avaliacaodiaria_operador'), 
                'o.id = u.avaliacao_diaria', 
                array('nome_operador'),
                'LEFT'
                );
            ...
            if(isset($params['empresa']) && !empty($params['empresa'])){    
                $select->where(array('tb_avaliacaodiaria.empresa' => $params['empresa']));
            }

            if(isset($params['usuario']) && !empty($params['usuario'])){    
                $select->where(array('tb_avaliacaodiaria.usuario' => $params['usuario']));
            }
    ...
    public function getAvaliacoesByMesEmpresa($empresa, $mes){
        return $this->getTableGateway()->select(function($select) use ($empresa, $mes) {
            ...
            $select->where(array('tb_avaliacaodiaria.empresa' => $empresa));
    ...
    public function getAvaliacacoById($idAvaliacao){
        return $this->getTableGateway()->select(function($select) use ($idAvaliacao) {
            ...
            $select->where(array('tb_avaliacaodiaria.id' => $idAvaliacao));
        })->current(); 
    }
    ...
    public function getAvaliacoesByParams($params){
        return $this->getTableGateway()->select(function($select) use ($params) {
            ...
            $select->where('tb_avaliacaodiaria.data_referencia >= "'.$params['inicio'].' 00:00:00"');
            $select->where('tb_avaliacaodiaria.data_referencia <= "'.$params['fim'].' 23:59:59"'); 
    ...
    public function getAvaliacoesGraficoPersonalizado($empresas, $campos, $datas){
        return $this->getTableGateway()->select(function($select) use ($empresas, $campos, $datas) {
            ...
            $select->where->notEqualTo('auditado', 'S');
            $select->where('tb_avaliacaodiaria.data_referencia >= "'.$datas['inicio'].' 00:00:00"');
            $select->where('tb_avaliacaodiaria.data_referencia <= "'.$datas['fim'].' 23:59:59"'); 
    ...
    public function getAvaliacoesToExcell($params){
        return $this->getTableGateway()->select(function($select) use ($params) {
            $select->join(
                array('e' => 'tb_empresa'), 
                'e.id = tb_avaliacaodiaria.empresa', 
                array('nome_empresa', 'id_empresa' => 'id', 'nome_responsavel'));
    ...
    public function getAvaliacoesAbertasByOperador($params){
        $adapter = $this->tableGateway->getAdapter();
        $where = '';
        $empresa = $adapter->platform->quoteIdentifier($params['empresa']);
        
        $sql = 'SELECT pa.* 
                FROM tb_pilha_avaliacoes_avaliacaodiaria AS pa 
                WHERE pa.data_referencia NOT IN(
                    SELECT data_referencia 
                    FROM tb_avaliacaodiaria 
